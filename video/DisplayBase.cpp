 ...
 * @retval      None
******************************************************************************/
DisplayBase::DisplayBase( controller_ch_t ch )
{
    /* Lcd setting (default) */
...
DisplayBase::Graphics_init( const lcd_config_t * lcd_config )
{
    ...
    return (graphics_error_t)DRV_Graphics_Init( (drv_ch_t)_vdc5_ch, (drv_lcd_config_t *)&_lcd_config );
...
DisplayBase::Graphics_Video_init( video_input_sel_t video_input_sel, video_ext_in_config_t * video_ext_in_config )
{
    ...
    if ((video_input_sel == INPUT_SEL_EXT)
     || (video_input_sel == INPUT_SEL_CEU)) {
        ...
        error = (graphics_error_t)DRV_Graphics_Video_init( (drv_ch_t)_vdc5_ch, (drv_video_input_sel_t)video_input_sel,
...
{
    return (graphics_error_t)DRV_Graphics_Lcd_Port_Init( (drv_ch_t)_vdc5_ch, pin, pin_count );
...
{
    return (graphics_error_t)DRV_Graphics_Dvinput_Port_Init( (drv_ch_t)_vdc5_ch, pin, pin_count );
...
{
    return (graphics_error_t)DRV_Graphics_Irq_Handler_Set( (drv_ch_t)_vdc5_ch, (vdc5_int_type_t)irq, num, (void (*)(vdc5_int_type_t))callback );
...
{
    return (graphics_error_t)DRV_Graphics_Start( (drv_ch_t)_vdc5_ch, (drv_graphics_layer_t)layer_id );
...
{
    return (graphics_error_t)DRV_Graphics_Stop( (drv_ch_t)_vdc5_ch, (drv_graphics_layer_t)layer_id );
...
DisplayBase::Video_Start( video_input_channel_t video_input_channel )
{
    ...
    if( error == GRAPHICS_OK ) {
        error = (graphics_error_t)DRV_Video_Start( (drv_ch_t)_vdc5_ch, (drv_video_input_channel_t)video_input_channel );
    }
    return error;
...
DisplayBase::Video_Stop( video_input_channel_t video_input_channel )
{
    ...
    if( error == GRAPHICS_OK ) {
        error = (graphics_error_t)DRV_Video_Stop( (drv_ch_t)_vdc5_ch, (drv_video_input_channel_t)video_input_channel );
    }
    return error;
