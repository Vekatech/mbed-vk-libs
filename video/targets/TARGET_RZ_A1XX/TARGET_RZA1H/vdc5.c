...
static const IRQn_Type vdc5_irq_set_tbl[VDC5_CHANNEL_NUM][VDC5_INT_TYPE_NUM] = {
{
    S0_VI_VSYNC0_IRQn,
    S0_LO_VSYNC0_IRQn,
    S0_VSYNCERR0_IRQn,
...
static void DRV_Graphics_Irq_Set(drv_ch_t drv_ch, vdc5_int_type_t irq, uint32_t enable);
...
static void init_func (const uint32_t user_num)
{
    ...
    if((vdc5_channel_t)user_num == VDC5_CHANNEL_1)
    {
        /* Standby control register 9 (STBCR9)
...
static void DRV_Graphics_Irq_Set(drv_ch_t drv_ch, vdc5_int_type_t irq, uint32_t enable)
{
    vdc5_channel_t          ch = (vdc5_channel_t)drv_ch; //VDC5_CHANNEL_0;
    IRQn_Type IRQn;
    IRQHandler handler;

    IRQn = vdc5_irq_set_tbl[ch][irq];
    ...
    void (* callback)(vdc5_int_type_t)  )
{
    ...
    if ((_drv_video_input_sel == DRV_INPUT_SEL_CEU) && (irq == VDC5_INT_TYPE_S0_VFIELD)) {
        ...
        if( callback == NULL ) {
            DRV_Graphics_Irq_Set( drv_ch, irq, 0 );
        } else {
            DRV_Graphics_Irq_Set( drv_ch, irq, 1 );
        }
 ...
 * @retval      Error code
******************************************************************************/
drv_graphics_error_t DRV_Graphics_Lcd_Port_Init( drv_ch_t drv_ch, PinName *pin, uint32_t pin_count )
{
    drv_graphics_error_t    drv_error   = DRV_GRAPHICS_OK;
    uint32_t count;
 ...
 * @retval      Error code
******************************************************************************/
drv_graphics_error_t DRV_Graphics_Dvinput_Port_Init( drv_ch_t drv_ch, PinName *pin, uint32_t pin_count )
{
    drv_graphics_error_t    drv_error   = DRV_GRAPHICS_OK;
    uint32_t count;
 ...
 * @retval      Error code
******************************************************************************/
drv_graphics_error_t DRV_Graphics_Init(drv_ch_t drv_ch, drv_lcd_config_t * drv_lcd_config )
{
    drv_graphics_error_t    drv_error   = DRV_GRAPHICS_OK;
    vdc5_channel_t          ch          = (vdc5_channel_t)drv_ch; //VDC5_CHANNEL_0;
    vdc5_error_t            error;
    ...
    init.panel_icksel   = LCD_CH0_PANEL_CLK;      /* Panel clock select */
    init.panel_dcdr     = LCD_CH0_PANEL_CLK_DIV;  /* Panel clock frequency division ratio */
    ...
    if( drv_lcd_config != NULL ) {
        ...
        LvdsUsed = (drv_lcd_config->lcd_type == DRV_LCD_TYPE_LVDS)? LVDS_IF_USE : LVDS_IF_NOT_USE;   /* LVDS PLL Setting Calculation */
    }
 ...
 * @retval      Error code
******************************************************************************/
drv_graphics_error_t DRV_Graphics_Video_init( drv_ch_t drv_ch, drv_video_input_sel_t drv_video_input_sel, drv_video_ext_in_config_t * drv_video_ext_in_config )
{
    drv_graphics_error_t    drv_error   = DRV_GRAPHICS_OK;
    vdc5_error_t            error;
    vdc5_input_t            input;
    ...
    /* Video input 0ch */
    error = R_VDC5_VideoInput( (vdc5_channel_t)drv_ch, &input );
    if (error != VDC5_OK) {
        drv_error = DRV_GRAPHICS_VDC5_ERR;
    }
...
******************************************************************************/
drv_graphics_error_t DRV_Graphics_Start ( drv_ch_t drv_ch, drv_graphics_layer_t layer_id )
{
    drv_graphics_error_t    drv_error   = DRV_GRAPHICS_OK;
    vdc5_channel_t          ch          = (vdc5_channel_t)drv_ch; //VDC5_CHANNEL_0;
    vdc5_error_t            error;
 ...
 * @retval      Error code
******************************************************************************/
drv_graphics_error_t DRV_Graphics_Stop ( drv_ch_t drv_ch, drv_graphics_layer_t layer_id )
{
    drv_graphics_error_t    drv_error   = DRV_GRAPHICS_OK;
    vdc5_channel_t          ch          = (vdc5_channel_t)drv_ch; //VDC5_CHANNEL_0;
    vdc5_error_t            error;
 ...
 * @retval      Error code
******************************************************************************/
drv_graphics_error_t DRV_Video_Start ( drv_ch_t drv_ch, drv_video_input_channel_t video_input_ch )
{
    drv_graphics_error_t    drv_error   = DRV_GRAPHICS_OK;
 ...
 * @retval      Error code
******************************************************************************/
drv_graphics_error_t DRV_Video_Stop ( drv_ch_t drv_ch, drv_video_input_channel_t video_input_ch )
{
    drv_graphics_error_t    drv_error   = DRV_GRAPHICS_OK;
    ...
    uint32_t                      fb_stride )
{
    ...
    else
    {
        if( video_input_ch == DRV_VIDEO_INPUT_CHANNEL_0 ) {
            ...
            VDC51.SC1_SCL1_WR2 = (uint32_t)framebuffer_t;
            VDC51.SC1_SCL1_WR8 = (uint32_t)framebuffer_b;
            VDC51.SC1_SCL1_UPDATE = 0x10;
        } else {
            drv_error = DRV_GRAPHICS_CHANNEL_ERR;
        }
    }
