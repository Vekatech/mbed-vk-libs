...
public:
    ...
    virtual bool Initialise() {
        ...
        */
        ...
        for (uint32_t i = 0; i < (sizeof(OV7725_InitRegTable) / sizeof(OV7725_InitRegTable[0])) ; i++) {
            ret = mI2c_.write(OV7725_I2C_SLAVE_ADDR, OV7725_InitRegTable[i], sizeof(OV7725_InitRegTable[i])/sizeof(OV7725_InitRegTable[i][0]));
            if (ret != 0) {
                return false;
            }
        }
    ...
    static bool SetExposure(bool bAuto, uint16_t usManualExposure, uint8_t usManualGain) {
        ...
        cmd[0] = 0x13;
        ret = mI2c_.write(OV7725_I2C_SLAVE_ADDR, &cmd[0], 1);
        if (ret != 0) {
            return false;
        }
        ret = mI2c_.read(OV7725_I2C_SLAVE_ADDR, &cmd[1], 1);
        if (ret != 0) {
            return false;
        }
        ...
        ret = mI2c_.write(OV7725_I2C_SLAVE_ADDR, &cmd[0], 2);
        if (ret != 0) {
            return false;
        }
